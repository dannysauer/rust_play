...
fn main() {
  let cmds = [
    ProcessArgs{ cmd: "/bin/echo", args: Some("hello world") },
    ProcessArgs{ cmd: "/bin/echoo", args: Some("hello world") },
    ProcessArgs{ cmd: "/bin/date", args: None },
    ];
...
fn t_child_launcher(cmd_params:&ProcessArgs) {
  ...
  let mut kid = cmd.spawn().expect(&format!("failed to run '{}'", cmd_params.cmd));
